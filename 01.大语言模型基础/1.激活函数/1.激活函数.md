...
谓激活函数（Activation Function），就是在[人工神经网络](https://baike.baidu.com/item/人工神经网络/382460?fromModule=lemma_inlink)的神经元上运行的[函数](https://baike.baidu.com/item/函数/301912?fromModule=lemma_inlink)，负责将神经元的输入映射到输出端

神经网络是线性的，无法解决非线性的问题（即变量之间的数学关系，不是直线而是曲线、曲面、或不确定的属性，叫非线性），加入激活函数就是给模型引入非线性能力；
...
2. ReLU，全称为：Rectified Linear Unit，是一种人工神经网络中常用的激活函数，通常意义下，其指代数学中的斜坡函数

   ![image-20250114162311639](/Users/apple/Documents/llm_interview_note/01.大语言模型基础/1.激活函数/image/image-20250114162311639.png)

3. 上述第二个问题没有能够完全解决的方法，目前有一些方法可以很大程度上进行缓解该问题，比如：对梯度做截断解决梯度爆炸问题、残差连接、normalize。由于使用了残差连接和 normalize 之后梯度消失和梯度爆炸已经极少出现了，所以目前可以认为该问题已经解决了。
